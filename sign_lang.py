...
def image_save(image, image_count, row_start, row_end, column_start, column_end):
    ...
    rect_image = image[int(row_start*ORIG_ROWS/ROWS):int(row_end*ORIG_ROWS/ROWS),
                    int(column_start*ORIG_COLS/COLS):int(column_end*ORIG_COLS/COLS)]
    try:
...
def main():
    ...
    vid = cv2.VideoCapture('vid1_cropped.mp4')
    ...
    while(True):
        ...
        image_count += 1 
        # turn to gray, and resize down to COLSxROWS
        frame = cv2.resize(cv2.cvtColor(frame_temp,
                                         cv2.COLOR_BGR2GRAY),
                                         (COLS,ROWS),
                                         interpolation=cv2.INTER_AREA)

        # read max_val_location from signal processing thread
        frame_final = copy.deepcopy(frame_temp)
        if max_val_location is not None:
            ...
            frame_final = cv2.rectangle(frame_temp,
